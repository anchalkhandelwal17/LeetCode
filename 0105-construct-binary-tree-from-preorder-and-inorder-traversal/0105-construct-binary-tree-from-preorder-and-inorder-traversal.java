...
class Solution {
    ...
    public TreeNode tree(int[] preorder, int preStart, int preEnd, int[] inorder, int inStart, int inEnd, HashMap<Integer, Integer> map){
        ...
        root.left = tree(preorder, preStart + 1, preStart + numsLeft, inorder, inStart, inRoot- 1, map);

        root.right = tree(preorder, preStart + numsLeft + 1 , preEnd, inorder, inRoot + 1, inEnd, map);
